...
function buildPatterns(receipts: Receipt[], now = new Date()) {
  ...
  byClient.forEach((client, key) => {
    ...
    const monthEnd = monthEndVotes >= Math.max(1, Math.ceil(samples.length / 2));
...
function notesForTargetMonth(notes: OpenReceivable[], targetMonth: Date, now = new Date()) {
  ...
  return notes.filter((note) => {
    ...
    if (!due) return targetKey === monthKey(currentMonth);
...
function buildActuals(receipts: Receipt[], weeks: ForecastWeek[]) {
  ...
  receiptSource(receipts).forEach((receipt) => {
    ...
    const current = actuals.get(key) ?? { total: 0, dates: [], weekId: week.id };
...
function buildForecastRows(
  ...
  actuals.forEach((actual, compositeKey) => {
    ...
    rows.push({
      ...
      clientKey,
      clientName: pattern?.clientName ?? clientKey,
      status: "Recebido",
      value: actual.total,
  ...
  patternsResult.patterns.forEach((pattern, clientKey) => {
    ...
    if (hasCurrentActual && !notes.length) return;

    const { center, start, end } = projectPattern(pattern, targetMonth);
    ...
    const origin: Origin = notes.length ? "Nota em aberto + histórico" : "Somente histórico";
    const observation = notes.length
      ? pattern.observation
      : `${pattern.observation} Nota ainda não localizada na CONTAS A RECEBER.`;
    ...
    rows.push({
      ...
      averageDueDifference: averageDueDifference(receipts, notes),
...
function ForecastView({ data }: { data: ImportState }) {
  ...
  function activateScope(scope: CardScope) {
    const next = activeScope === scope ? "all" : scope;
    setActiveScope(next);
  ...
  if (!data.receipts.length) {
    return (
      <section className="receipt-forecast-page-v11">
        <div className="forecast-empty-v11">
          <CalendarClock size={44} />
          <span>PREVISÃO DE RECEBIMENTOS</span>
          <h2>Importe a planilha de Recebimentos.</h2>
          <p>A previsão será formada somente pelos recebimentos reais dos quatro meses completos anteriores.</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="receipt-forecast-page-v11">
      <div className="forecast-heading-v11">
        <div>
          <span>HISTÓRICO REAL DOS ÚLTIMOS 4 MESES</span>
          <h2>Previsão de Recebimentos</h2>
          <p>
            Clientes e datas são definidos exclusivamente pelos recebimentos reais de {patternsResult.periodLabel}. A CONTAS A RECEBER apenas complementa nota, saldo e vencimento.
          </p>
        </div>
        <div className="forecast-method-v11">
          <CalendarClock size={20} />
          <div><strong>Sem previsão por vencimento isolado</strong><span>Cliente sem recebimento recente não entra</span></div>
        </div>
      </div>

      {openSheetState === "missing" ? (
        <div className="forecast-warning-v11">
          <FileQuestion size={20} />
          <div>
            <strong>A aba CONTAS A RECEBER não foi localizada.</strong>
            <span>A previsão histórica continua válida, mas notas, saldos e vencimentos não serão exibidos.</span>
          </div>
        </div>
      ) : null}
      {openSheetState === "loading" ? (
        <div className="forecast-warning-v11 neutral"><ReceiptText size={20} /><span>Lendo a aba CONTAS A RECEBER como informação auxiliar…</span></div>
      ) : null}

      <section className="forecast-filters-v11">
      ...
      <section className="forecast-kpis-v11">
        <article className={activeScope === "forecast" ? "active" : ""} role="button" tabIndex={0} onClick={() => activateScope("forecast")} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") activateScope("forecast"); }}><span>Previsão a receber</span><strong>{currency.format(predictedValue)}</strong><small>{predictedClients} clientes com recebimento nos últimos 4 meses</small></article>
        <article className={activeScope === "received" ? "active" : ""} role="button" tabIndex={0} onClick={() => activateScope("received")} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") activateScope("received"); }}><span>Recebido no período</span><strong>{currency.format(receivedValue)}</strong><small>{new Set(receivedRows.map((row) => row.clientKey)).size} clientes com valor real lançado</small></article>
        <article className={activeScope === "open" ? "active" : ""} role="button" tabIndex={0} onClick={() => activateScope("open")} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") activateScope("open"); }}><span>Notas abertas vinculadas</span><strong>{currency.format(openNoteValue)}</strong><small>{openRows.reduce((sum, row) => sum + row.notes.length, 0)} notas auxiliares relacionadas</small></article>
        <article className={activeScope === "history" ? "active" : ""} role="button" tabIndex={0} onClick={() => activateScope("history")} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") activateScope("history"); }}><span>Previsões históricas sem nota</span><strong>{integer.format(historyRows.length)}</strong><small>Clientes recentes sem nota aberta localizada</small></article>
      </section>

      <section className="forecast-main-v11">
        <article className="forecast-panel-v11">
          <div className="forecast-panel-head-v11"><div><h3>Histórico real dos clientes filtrados</h3><p>Somente valores efetivamente recebidos nos quatro meses analisados</p></div></div>
          <div className="forecast-chart-v11"><ResponsiveContainer width="100%" height="100%"><BarChart data={historyChart} margin={{ top: 12, right: 8, left: 4, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" stroke="#e8ebf2" vertical={false} /><XAxis dataKey="month" tickLine={false} axisLine={false} tick={{ fill: "#788198", fontSize: 12 }} /><YAxis tickFormatter={(value: number | string) => compactCurrency.format(Number(value))} tickLine={false} axisLine={false} tick={{ fill: "#9aa2b3", fontSize: 11 }} width={72} /><Tooltip formatter={(value: number | string) => currency.format(Number(value))} /><Bar dataKey="amount" name="Recebido" fill="#5d72f6" radius={[6, 6, 0, 0]} maxBarSize={54} /></BarChart></ResponsiveContainer></div>
        </article>
        <article className="forecast-panel-v11">
          <div className="forecast-panel-head-v11"><div><h3>Semanas econômicas de {MONTH_FORMATTER.format(selectedMonth)}</h3><p>A última semana continua no mês seguinte quando necessário</p></div></div>
          <div className="forecast-weeks-v11">{weeklySummary.map((week) => <button key={week.id} type="button" className={selectedWeek === week.id ? "active" : ""} onClick={() => { setSelectedWeek((current) => current === week.id ? "all" : week.id); setSelectedClient("all"); }}><span>{week.label}</span><strong>A receber: {currency.format(week.forecast)}</strong><em>Recebido: {currency.format(week.received)}</em><small>{week.forecastClients} previstos · {week.receivedClients} recebidos</small><i>{week.names.length ? week.names.join(" · ") : "Sem previsão pendente"}</i></button>)}</div>
        </article>
      </section>

      <article className="forecast-panel-v11" ref={tableRef}>
        <div className="forecast-panel-head-v11"><div><h3>Análise individual por cliente</h3><p>{displayedRows.length} registros no card e filtros atuais</p></div>{activeScope !== "all" ? <button type="button" className="scope-clear" onClick={() => setActiveScope("all")}>Remover filtro do card</button> : null}</div>
        <div className="forecast-table-v11"><table><thead><tr><th>Cliente</th><th className="number">Valor</th><th>Data ou semana provável</th><th>Notas em aberto</th><th>Vencimentos</th><th>Faixa histórica</th><th>Meses</th><th>Dif. vencimento</th><th>Confiança</th><th>Origem</th><th>Observação</th></tr></thead><tbody>{displayedRows.map((row) => <tr key={row.id} onClick={() => setSelectedDetailId((current) => current === row.id ? null : row.id)}><td className="client"><strong>{row.clientName}</strong><span>{row.status === "Recebido" ? <><CheckCircle2 size={12} /> Recebido</> : "Previsão pendente"}</span></td><td className="number"><strong>{currency.format(row.value)}</strong></td><td><strong>{row.status === "Recebido" ? row.actualDates.map(formatDate).join(", ") : `${formatDate(row.rangeStart)} a ${formatDate(row.rangeEnd)}`}</strong><span>{row.weekLabel}</span></td><td><span>{notesLabel(row.notes)}</span></td><td><span>{row.dueDates.length ? row.dueDates.map(formatDate).join(", ") : "—"}</span></td><td><span>{row.historicalRange}</span></td><td>{row.monthsUsed}/4</td><td>{averageDifferenceLabel(row.averageDueDifference)}</td><td><span className={`confidence ${confidenceClass(row.confidence)}`}>{row.confidence}</span></td><td><span className={`origin ${originClass(row.origin)}`}>{row.origin}</span></td><td className="observation">{row.observation}</td></tr>)}{!displayedRows.length ? <tr><td colSpan={11} className="empty-row">Nenhum cliente encontrado para o card e os filtros selecionados.</td></tr> : null}</tbody></table></div>
      </article>

      {selectedDetail ? <article className="forecast-panel-v11 forecast-detail-v11"><div className="forecast-panel-head-v11"><div><h3>{selectedDetail.clientName}</h3><p>Memória de cálculo da previsão selecionada</p></div><button type="button" onClick={() => setSelectedDetailId(null)}>Fechar</button></div><div className="detail-grid"><div><span>Origem</span><strong>{selectedDetail.origin}</strong></div><div><span>Valor considerado</span><strong>{currency.format(selectedDetail.value)}</strong></div><div><span>Janela</span><strong>{formatDate(selectedDetail.rangeStart)} a {formatDate(selectedDetail.rangeEnd)}</strong></div><div><span>Histórico</span><strong>{selectedDetail.historicalRange}</strong></div><div><span>Notas auxiliares</span><strong>{notesLabel(selectedDetail.notes)}</strong></div><div><span>Observação</span><strong>{selectedDetail.observation}</strong></div></div></article> : null}

      <div className="forecast-note-v11"><CircleAlert size={17} /><span>A CONTAS A RECEBER nunca cria uma previsão. O cliente precisa ter recebimento real em pelo menos um dos quatro meses completos analisados. Vencimentos e saldos servem somente para complementar a linha já prevista pelo histórico.</span></div>

      <style jsx global>{`
        .receipt-forecast-active .content-area > :not(.receipt-forecast-page-v11) { display: none !important; }
        .receipt-forecast-page-v11 { display: grid; gap: 20px; color: #20263a; animation: forecastV11In .18s ease-out; }
        @keyframes forecastV11In { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: none; } }
        .forecast-heading-v11 { display: flex; justify-content: space-between; gap: 22px; align-items: flex-start; }
        .forecast-heading-v11 > div:first-child > span { color: #5d72f6; font-size: 10px; font-weight: 800; letter-spacing: .11em; }
        .forecast-heading-v11 h2 { margin: 5px 0 0; font-size: clamp(25px, 2.4vw, 34px); }
        .forecast-heading-v11 p { max-width: 820px; margin: 7px 0 0; color: #788198; line-height: 1.55; font-size: 13px; }
        .forecast-method-v11 { min-width: 285px; display: flex; gap: 10px; align-items: center; padding: 14px 16px; border: 1px solid #dfe4f1; border-radius: 14px; background: #fff; color: #5d72f6; }
        .forecast-method-v11 div { display: grid; gap: 2px; }.forecast-method-v11 strong { color: #27304b; font-size: 12px; }.forecast-method-v11 span { color: #8790a4; font-size: 10px; }
        .forecast-warning-v11,.forecast-note-v11 { display: flex; gap: 10px; align-items: flex-start; padding: 13px 15px; border: 1px solid #f0dcae; border-radius: 12px; background: #fffaf0; color: #805f18; font-size: 12px; line-height: 1.5; }
        .forecast-warning-v11 div { display: grid; gap: 3px; }.forecast-warning-v11.neutral { border-color: #dfe4f1; background: #f8f9fd; color: #59647b; align-items: center; }
        .forecast-filters-v11 { display: flex; gap: 10px; align-items: end; flex-wrap: wrap; padding: 15px; border: 1px solid #e2e6ef; border-radius: 15px; background: #fff; }
        .forecast-filters-v11 .filter-title { display: grid; min-width: 150px; margin-right: auto; }.forecast-filters-v11 .filter-title span { font-weight: 800; font-size: 12px; }.forecast-filters-v11 .filter-title small { color: #929aac; font-size: 9px; }
        .forecast-filters-v11 label { display: grid; gap: 5px; }.forecast-filters-v11 label > span { color: #7d869a; font-size: 9px; font-weight: 800; text-transform: uppercase; }.forecast-filters-v11 label > div { display: flex; align-items: center; gap: 5px; height: 37px; padding: 0 9px; border: 1px solid #dfe4ee; border-radius: 9px; }.forecast-filters-v11 select { min-width: 145px; border: 0; outline: 0; appearance: none; background: transparent; color: #374057; font: inherit; font-size: 11px; }.forecast-filters-v11 button,.forecast-detail-v11 button,.scope-clear { height: 37px; padding: 0 12px; border: 1px solid #dfe4ee; border-radius: 9px; background: #fff; color: #626c83; font-size: 11px; font-weight: 800; cursor: pointer; }
        .forecast-kpis-v11 { display: grid; grid-template-columns: repeat(4,minmax(0,1fr)); gap: 13px; }.forecast-kpis-v11 article { display: grid; min-height: 118px; align-content: space-between; padding: 17px; border: 1px solid #e4e8f1; border-radius: 15px; background: #fff; cursor: pointer; transition: .15s ease; }.forecast-kpis-v11 article:hover,.forecast-kpis-v11 article:focus-visible { border-color: #aeb9ff; box-shadow: 0 12px 30px rgba(71,88,190,.12); outline: none; transform: translateY(-1px); }.forecast-kpis-v11 article.active { border-color: #5d72f6; background: #f7f8ff; box-shadow: 0 0 0 2px rgba(93,114,246,.13); }.forecast-kpis-v11 span { color: #707a91; font-size: 10px; font-weight: 800; }.forecast-kpis-v11 strong { font-size: clamp(19px,2vw,27px); }.forecast-kpis-v11 small { color: #929aac; font-size: 9px; }
        .forecast-main-v11 { display: grid; grid-template-columns: minmax(0,1.4fr) minmax(350px,.8fr); gap: 15px; }.forecast-panel-v11 { overflow: hidden; border: 1px solid #e4e8f1; border-radius: 15px; background: #fff; }.forecast-panel-head-v11 { display: flex; justify-content: space-between; gap: 12px; align-items: center; padding: 17px 19px 13px; }.forecast-panel-head-v11 h3 { margin: 0; font-size: 14px; }.forecast-panel-head-v11 p { margin: 4px 0 0; color: #8d95a8; font-size: 10px; }.forecast-chart-v11 { height: 285px; padding: 4px 12px 14px; }
        .forecast-weeks-v11 { display: grid; max-height: 310px; overflow: auto; }.forecast-weeks-v11 button { display: grid; gap: 3px; padding: 12px 14px; border: 0; border-bottom: 1px solid #edf0f5; background: #fff; text-align: left; cursor: pointer; }.forecast-weeks-v11 button.active { background: #f1f3ff; box-shadow: inset 3px 0 #5d72f6; }.forecast-weeks-v11 span { color: #677188; font-size: 10px; font-weight: 800; }.forecast-weeks-v11 strong { font-size: 12px; }.forecast-weeks-v11 em { color: #16866f; font-size: 10px; font-style: normal; font-weight: 700; }.forecast-weeks-v11 small { color: #8a93a7; font-size: 9px; }.forecast-weeks-v11 i { overflow: hidden; color: #9aa2b3; font-size: 8px; font-style: normal; text-overflow: ellipsis; white-space: nowrap; }
        .forecast-table-v11 { overflow-x: auto; border-top: 1px solid #edf0f5; }.forecast-table-v11 table { width: 100%; min-width: 1780px; border-collapse: collapse; }.forecast-table-v11 th,.forecast-table-v11 td { padding: 12px 13px; border-bottom: 1px solid #edf0f5; text-align: left; vertical-align: top; font-size: 10px; }.forecast-table-v11 th { background: #fafbfe; color: #7d869a; font-size: 9px; text-transform: uppercase; }.forecast-table-v11 th.number,.forecast-table-v11 td.number { text-align: right; }.forecast-table-v11 tbody tr { cursor: pointer; }.forecast-table-v11 tbody tr:hover { background: #f8f9ff; }.forecast-table-v11 td.client { display: grid; min-width: 235px; gap: 4px; }.forecast-table-v11 td.client strong { font-size: 11px; }.forecast-table-v11 td.client span { display: flex; gap: 4px; align-items: center; color: #8d95a8; font-size: 9px; }.forecast-table-v11 td > span { display: block; max-width: 210px; color: #626c83; line-height: 1.4; }.forecast-table-v11 td:nth-child(3) { min-width: 190px; }.forecast-table-v11 td:nth-child(3) span { margin-top: 3px; color: #929aac; font-size: 8px; }.forecast-table-v11 .observation { min-width: 245px; line-height: 1.45; }.confidence,.origin { display: inline-flex !important; width: fit-content; padding: 5px 8px; border-radius: 999px; font-size: 8px !important; font-weight: 800; white-space: nowrap; }.confidence.alta { background: #e8f8f3; color: #16866f !important; }.confidence.media { background: #eef0ff; color: #5367df !important; }.confidence.baixa { background: #fff4dd; color: #9b6c08 !important; }.origin.both { background: #e8f8f3; color: #16866f !important; }.origin.history { background: #eef0ff; color: #5367df !important; }.origin.actual { background: #e9f5ff; color: #2878a9 !important; }.empty-row { padding: 28px !important; text-align: center !important; color: #8d95a8; }
        .forecast-detail-v11 { padding-bottom: 16px; }.detail-grid { display: grid; grid-template-columns: repeat(3,minmax(0,1fr)); gap: 10px; padding: 0 19px; }.detail-grid div { display: grid; gap: 5px; padding: 12px; border: 1px solid #e7eaf2; border-radius: 10px; background: #fbfcff; }.detail-grid span { color: #8790a4; font-size: 9px; text-transform: uppercase; }.detail-grid strong { color: #3e475e; font-size: 11px; line-height: 1.4; }
        .forecast-empty-v11 { display: grid; min-height: 450px; place-items: center; align-content: center; gap: 9px; padding: 35px; border: 1px dashed #cfd6e6; border-radius: 18px; background: #fff; color: #5d72f6; text-align: center; }.forecast-empty-v11 span { font-size: 10px; font-weight: 800; letter-spacing: .11em; }.forecast-empty-v11 h2 { margin: 0; color: #27304b; }.forecast-empty-v11 p { max-width: 610px; margin: 0; color: #7f889d; }
        @media (max-width: 1150px) { .forecast-kpis-v11 { grid-template-columns: repeat(2,minmax(0,1fr)); }.forecast-main-v11 { grid-template-columns: 1fr; } }
        @media (max-width: 760px) { .forecast-heading-v11 { display: grid; }.forecast-method-v11 { min-width: 0; }.forecast-kpis-v11 { grid-template-columns: 1fr; }.forecast-filters-v11 .filter-title { flex: 1 1 100%; }.forecast-filters-v11 label { flex: 1 1 100%; }.forecast-filters-v11 select { width: 100%; }.detail-grid { grid-template-columns: 1fr; } }
      `}</style>
    </section>
  );
}

export default function ReceiptForecastEnhancerV11() {
