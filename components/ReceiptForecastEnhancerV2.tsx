...
function confidenceFor(activeMonths: number, windowSpan: number): Confidence {
  ...
  if (activeMonths >= 4 && windowSpan <= 7) return "Alta";
  if (activeMonths >= 3 && windowSpan <= 12) return "Média";
  return "Baixa";
}
...
function buildForecastAnalysis(receipts: Receipt[], now = new Date()): ForecastAnalysis {
  ...
  sourceReceipts.forEach((receipt) => {
    ...
    const accumulator = byClient.get(key) ?? {
      ...
      monthlyDailyTotals: Array.from({ length: 6 }, () => new Map<number, number>()),
      receiptCount: 0,
    };
  ...
  const clients = [...byClient.entries()].map(([key, accumulator]): ClientForecast => {
    ...
    const monthlyEstimate = activeMonthlyTotals.length ? median(activeMonthlyTotals) : 0;
    ...
    return {
      ...
      endDay,
      monthlyEstimate,
      historyTotal: accumulator.monthlyTotals.reduce((sum, value) => sum + value, 0),
      activeMonths,
...
function ReceiptForecastView({ data }: { data: ImportState }) {
  ...
  if (!data.receipts.length) {
    return (
      <section className="receipt-forecast-page-v2">
        <div className="forecast-empty-v2">
  ...
  return (
    <section className="receipt-forecast-page-v2">
      <div className="forecast-heading-v2">
        <div>
          <span>HISTÓRICO AUTOMÁTICO</span>
          <h2>Previsão de Recebimentos</h2>
          <p>
            Previsão mensal e semanal calculada com os seis meses completos de {analysis.periodLabel}.
            Cada cliente é analisado separadamente.
          </p>
        </div>
      ...
      <section className="forecast-filter-bar-v2">
        <div className="forecast-filter-title-v2"><span>Filtros da previsão</span><small>Recalculam todos os valores abaixo</small></div>
        <label>
          <span>Cliente</span>
          <div className="forecast-select-v2">
        ...
        {(selectedClient !== "all" || selectedConfidence !== "Todas" || selectedWeek !== "all") ? (
          <button type="button" className="forecast-clear-v2" onClick={clearFilters}>Limpar</button>
        ) : null}
      </section>

      <section className="forecast-kpis-v2">
        ...
        <article><span>Alta confiança</span><strong>{currency.format(highConfidenceTotal)}</strong><small>Clientes regulares dentro dos filtros</small></article>
      ...
      <section className="forecast-main-grid-v2">
        <article className="forecast-panel-v2">
          <div className="forecast-panel-header-v2">
            <div><h3>Comparativo dos últimos seis meses</h3><p>Valores realmente recebidos, sem média mensal no gráfico</p></div>
          </div>
          <div className="forecast-chart-v2">
        ...
        <article className="forecast-panel-v2">
          ...
          <div className="forecast-weeks-v2">
      ...
      <article className="forecast-panel-v2">
        <div className="forecast-panel-header-v2 forecast-table-header-v2">
          <div><h3>Previsão por cliente</h3><p>{integer.format(filteredRows.length)} clientes após os filtros</p></div>
        </div>
        <div className="forecast-table-wrap-v2">
          <table>
            ...
            <tbody>
              {filteredRows.map((row) => (
                <tr key={row.key} onClick={() => setSelectedDetailKey((current) => current === row.key ? null : row.key)}>
                  <td className="forecast-client-v2"><strong>{row.clientName}</strong><span>Cliente analisado individualmente</span></td>
                  ...
                  <td>{row.activeMonths}/6 meses</td>
                  <td><span className={`forecast-trend-v2 ${row.trend.toLowerCase().replace(" ", "-")}`}>{trendIcon(row.trend)}{row.trend}</span></td>
                  <td><span className={`forecast-confidence-v2 ${confidenceClass(row.confidence)}`}>{row.confidence}</span></td>
                </tr>
              ))}
              {!filteredRows.length ? <tr><td colSpan={8} className="forecast-empty-row-v2">Nenhum cliente encontrado nos filtros selecionados.</td></tr> : null}
            </tbody>
          </table>
        </div>
      </article>
      ...
      {selectedDetail ? (
        <article className="forecast-panel-v2 forecast-detail-v2">
          <div className="forecast-panel-header-v2">
            <div>
              <h3>{selectedDetail.clientName}</h3>
              <p>
                Confiança baseada em {selectedDetail.activeMonths} meses com recebimento e nas datas representativas mensais: {selectedDetail.representativeDays.join(", ") || "sem dados"}.
              </p>
            </div>
            <button type="button" className="forecast-clear-v2" onClick={() => setSelectedDetailKey(null)}>Fechar detalhe</button>
          </div>
          <div className="forecast-history-grid-v2">
      ...
      <div className="forecast-note-v2">
        <CircleAlert size={17} />
        <span>
          O valor estimado de cada cliente usa a mediana dos valores mensais em que houve recebimento. A confiança usa uma data representativa por mês, evitando que vários lançamentos no mesmo dia reduzam artificialmente a confiança.
...
export default function ReceiptForecastEnhancerV2() {
  ...
  useEffect(() => {
    const syncTargets = () => {
      ...
      if (nav) {
        let mount = document.getElementById("receipt-forecast-nav-mount-v2");
        if (!mount) {
          mount = document.createElement("span");
          mount.id = "receipt-forecast-nav-mount-v2";
    ...
    return () => {
      observer.disconnect();
      document.getElementById("receipt-forecast-nav-mount-v2")?.remove();
    };
  }, []);
  ...
  useEffect(() => {
    const handleOtherNavigation = (event: MouseEvent) => {
      ...
      if (!target || target.dataset.forecastNavV2 === "true") return;
  ...
  useEffect(() => {
    document.body.classList.toggle("receipt-forecast-active-v2", active);
    if (active) {
      const title = document.querySelector<HTMLElement>(".topbar-title h1");
      if (title) title.textContent = "Previsão de Recebimentos";
    }
    return () => document.body.classList.remove("receipt-forecast-active-v2");
  }, [active]);
  ...
  return (
    <>
      ...
      <style jsx global>{`
        .receipt-forecast-active-v2 .content-area > :not(.receipt-forecast-page-v2) { display: none !important; }
        .receipt-forecast-page-v2 { display: grid; gap: 20px; animation: forecastV2Fade 180ms ease-out; }
        @keyframes forecastV2Fade { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: translateY(0); } }
        .forecast-heading-v2 { display: flex; justify-content: space-between; align-items: flex-start; gap: 22px; }
        .forecast-heading-v2 > div:first-child > span { display: block; margin-bottom: 6px; color: #5d72f6; font-size: 11px; font-weight: 800; letter-spacing: .11em; }
        .forecast-heading-v2 h2 { margin: 0; color: #20263a; font-size: clamp(24px, 2.2vw, 34px); }
        .forecast-heading-v2 p { max-width: 760px; margin: 7px 0 0; color: #788198; line-height: 1.55; }
        .forecast-method-v2 { display: flex; min-width: 270px; gap: 10px; align-items: center; padding: 14px 16px; border: 1px solid #dfe4f1; border-radius: 14px; background: #fff; color: #5d72f6; }
        .forecast-method-v2 div { display: grid; gap: 2px; }
        .forecast-method-v2 strong { color: #27304b; font-size: 13px; }
        .forecast-method-v2 span { color: #7e879d; font-size: 11px; }
        .forecast-filter-bar-v2 { display: grid; grid-template-columns: auto repeat(4, minmax(150px, 1fr)) auto; gap: 12px; align-items: end; padding: 16px; border: 1px solid #e3e7f0; border-radius: 15px; background: #fff; }
        .forecast-filter-title-v2 { display: grid; align-self: center; min-width: 145px; gap: 2px; }
        .forecast-filter-title-v2 span { color: #323b52; font-size: 12px; font-weight: 800; }
        .forecast-filter-title-v2 small { color: #959daf; font-size: 10px; }
        .forecast-filter-bar-v2 label { display: grid; gap: 6px; color: #778097; font-size: 10px; font-weight: 700; }
        .forecast-select-v2 { display: flex; height: 40px; align-items: center; gap: 7px; padding: 0 10px; border: 1px solid #dfe4ee; border-radius: 10px; background: #fff; color: #929aad; }
        .forecast-select-v2 select { width: 100%; min-width: 0; border: 0; outline: 0; appearance: none; background: transparent; color: #333b52; font: inherit; font-size: 12px; }
        .forecast-clear-v2 { min-height: 38px; padding: 0 12px; border: 1px solid #dfe4ee; border-radius: 9px; background: #fff; color: #626c83; font-size: 12px; font-weight: 700; cursor: pointer; }
        .forecast-kpis-v2 { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 14px; }
        .forecast-kpis-v2 article { display: grid; min-height: 128px; align-content: space-between; padding: 18px; border: 1px solid #e4e8f1; border-radius: 16px; background: #fff; }
        .forecast-kpis-v2 span { color: #727c93; font-size: 12px; font-weight: 700; }
        .forecast-kpis-v2 strong { color: #20263a; font-size: clamp(20px, 2vw, 28px); }
        .forecast-kpis-v2 small { color: #98a0b1; font-size: 11px; }
        .forecast-main-grid-v2 { display: grid; grid-template-columns: minmax(0, 1.35fr) minmax(350px, .95fr); gap: 16px; }
        .forecast-panel-v2 { overflow: hidden; border: 1px solid #e4e8f1; border-radius: 16px; background: #fff; }
        .forecast-panel-header-v2 { display: flex; justify-content: space-between; gap: 16px; align-items: center; padding: 18px 20px 0; }
        .forecast-panel-header-v2 h3 { margin: 0; color: #27304b; font-size: 15px; }
        .forecast-panel-header-v2 p { margin: 4px 0 0; color: #8d95a8; font-size: 11px; }
        .forecast-chart-v2 { height: 310px; padding: 10px 14px 14px; }
        .forecast-weeks-v2 { display: grid; gap: 9px; padding: 14px 18px 18px; }
        .forecast-weeks-v2 button { display: grid; gap: 4px; padding: 12px 14px; border: 1px solid #e4e8f1; border-radius: 12px; background: #fbfcff; text-align: left; cursor: pointer; }
        .forecast-weeks-v2 button:hover, .forecast-weeks-v2 button.active { border-color: #aeb9f4; background: #f2f4ff; }
        .forecast-weeks-v2 span { color: #586178; font-size: 11px; font-weight: 800; }
        .forecast-weeks-v2 strong { color: #20263a; font-size: 18px; }
        .forecast-weeks-v2 small { color: #8d95a8; font-size: 10px; }
        .forecast-weeks-v2 i { overflow: hidden; color: #707a91; font-size: 9px; font-style: normal; text-overflow: ellipsis; white-space: nowrap; }
        .forecast-table-header-v2 { padding-bottom: 14px; }
        .forecast-table-wrap-v2 { overflow-x: auto; border-top: 1px solid #edf0f5; }
        .forecast-table-wrap-v2 table { width: 100%; min-width: 1250px; border-collapse: collapse; }
        .forecast-table-wrap-v2 th, .forecast-table-wrap-v2 td { padding: 13px 15px; border-bottom: 1px solid #edf0f5; text-align: left; font-size: 12px; }
        .forecast-table-wrap-v2 th { background: #fafbfe; color: #7d869a; font-size: 10px; letter-spacing: .035em; text-transform: uppercase; }
        .forecast-table-wrap-v2 th.number, .forecast-table-wrap-v2 td.number { text-align: right; }
        .forecast-table-wrap-v2 tbody tr { cursor: pointer; }
        .forecast-table-wrap-v2 tbody tr:hover { background: #f7f8ff; }
        .forecast-client-v2 { display: grid; min-width: 240px; gap: 3px; }
        .forecast-client-v2 strong { color: #27304b; font-size: 12px; }
        .forecast-client-v2 span { color: #98a0b1; font-size: 10px; }
        .forecast-confidence-v2, .forecast-trend-v2 { display: inline-flex; align-items: center; gap: 5px; padding: 5px 8px; border-radius: 999px; font-size: 10px; font-weight: 800; white-space: nowrap; }
        .forecast-confidence-v2.alta { background: #e8f8f3; color: #16866f; }
        .forecast-confidence-v2.media { background: #eef0ff; color: #5367df; }
        .forecast-confidence-v2.baixa { background: #fff4dd; color: #9b6c08; }
        .forecast-confidence-v2.insuficiente { background: #f1f2f5; color: #7c8392; }
        .forecast-trend-v2 { padding-left: 0; background: transparent; color: #6e778d; }
        .forecast-trend-v2.em-alta { color: #16866f; }
        .forecast-trend-v2.em-queda { color: #c55767; }
        .forecast-history-grid-v2 { display: grid; grid-template-columns: repeat(6, minmax(0, 1fr)); gap: 10px; padding: 18px 20px 20px; }
        .forecast-history-grid-v2 div { display: grid; gap: 5px; padding: 12px; border: 1px solid #e7eaf2; border-radius: 11px; background: #fbfcff; }
        .forecast-history-grid-v2 span { color: #848da2; font-size: 10px; text-transform: uppercase; }
        .forecast-history-grid-v2 strong { color: #27304b; font-size: 13px; }
        .forecast-history-grid-v2 small { color: #a1a8b8; font-size: 9px; }
        .forecast-note-v2 { display: flex; gap: 10px; align-items: flex-start; padding: 14px 16px; border: 1px solid #f0dcae; border-radius: 12px; background: #fffaf0; color: #805f18; }
        .forecast-note-v2 span { font-size: 12px; line-height: 1.5; }
        .forecast-empty-row-v2 { padding: 28px !important; text-align: center !important; color: #8d95a8 !important; }
        .forecast-empty-v2 { display: grid; min-height: 460px; place-items: center; align-content: center; gap: 10px; padding: 38px; border: 1px dashed #cfd6e6; border-radius: 18px; background: #fff; color: #5d72f6; text-align: center; }
        .forecast-empty-v2 h2 { margin: 0; color: #27304b; }
        .forecast-empty-v2 p { max-width: 600px; margin: 0; color: #7f889d; }
        @media (max-width: 1250px) {
          .forecast-filter-bar-v2 { grid-template-columns: repeat(2, minmax(0, 1fr)); }
          .forecast-filter-title-v2 { grid-column: 1 / -1; }
          .forecast-kpis-v2 { grid-template-columns: repeat(2, minmax(0, 1fr)); }
          .forecast-main-grid-v2 { grid-template-columns: 1fr; }
          .forecast-history-grid-v2 { grid-template-columns: repeat(3, minmax(0, 1fr)); }
        }
        @media (max-width: 760px) {
          .forecast-heading-v2 { display: grid; }
          .forecast-method-v2 { min-width: 0; }
          .forecast-filter-bar-v2 { grid-template-columns: 1fr; }
          .forecast-kpis-v2 { grid-template-columns: 1fr; }
          .forecast-history-grid-v2 { grid-template-columns: repeat(2, minmax(0, 1fr)); }
          .forecast-chart-v2 { height: 270px; }
        }
      `}</style>
    </>
  );
