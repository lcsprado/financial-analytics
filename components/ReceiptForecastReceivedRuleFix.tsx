...
function readRow(row: HTMLTableRowElement): RowInfo | null {
  ...
  const predictedDate = row.dataset.adaptivePredictedDate || brDates(statusText.match(/Data provável:[^<]*/i)?.[0] || "")[0] || weekStart;
...
function matchCrossWeekReceipts(rows: RowInfo[]) {
  ...
  byClient.forEach((clientRows) => {
    ...
    actuals.forEach((actual) => {
      ...
      let best: RowInfo | null = null;
      let bestScore = Infinity;
      predictions.forEach((prediction) => {
        if (used.has(prediction.row)) return;
...
function refreshCardsAndWeeks() {
  ...
  document.querySelectorAll<HTMLButtonElement>(".forecast-weeks-v13 button").forEach((button) => {
    ...
    infos.forEach((info) => {
      ...
      const remaining = adaptiveActual > 0 && Number.isFinite(adaptiveRemaining) ? adaptiveRemaining : (!info.statusTitle.includes("Recebido") ? info.value : 0);
