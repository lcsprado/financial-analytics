  ...
  return <div className="dashboard-users-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="dashboard-users-panel" role="dialog" aria-modal="true" aria-label="Gerenciar usuários">
      ...
      <form className="dashboard-user-create" onSubmit={handleCreate}>
        <div className="dashboard-user-create-title"><UserPlus size={17}/><div><strong>Novo usuário</strong><span>Senha inicial: 1234, com troca obrigatória no primeiro acesso.</span></div></div>
        ...
        {loading ? <div className="dashboard-users-empty">Carregando usuários...</div> : users.map((user) => {
          ...
          return <article key={user.email} className={!user.active ? "is-disabled" : ""}>
            ...
            <div className="dashboard-user-actions">
              ...
              {!protectedOwner && user.active && <button type="button" className="refresh" disabled={refreshingEmail===user.email} onClick={()=>void refreshDashboard(user)}><RefreshCw size={15}/>{refreshingEmail===user.email?"Enviando...":"Atualizar dashboard"}</button>}
              {!protectedOwner && user.active && <button type="button" className="password" disabled={resettingEmail===user.email} onClick={()=>void resetPassword(user)}><KeyRound size={15}/>{resettingEmail===user.email?"Gerando...":"Nova senha 1234"}</button>}
